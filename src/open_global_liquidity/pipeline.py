...
from open_global_liquidity.analysis.signal_map import SignalMapError, build_liquidity_signal_map
...
def run_pipeline(
    *,
    ...
    signal_map: pd.DataFrame | None = None
    ...
    if (
        global_aggregate is not None
        and cross_border_indicators is not None
        and private_liquidity_indicators is not None
        and collateral_conditions is not None
    ):
        signal_map = build_liquidity_signal_map(
            global_aggregate,
            cross_border_indicators,
            private_liquidity_indicators,
            collateral_conditions,
        )
        signal_map.to_parquet(output_dir / "liquidity_signal_map.parquet", index=False)
        LOGGER.info("Wrote %d cross-channel signal-map rows", len(signal_map))
    ...
    if publish_dashboard_snapshot:
        ...
        if signal_map is not None:
            snapshots["liquidity_signal_map_snapshot.parquet"] = signal_map
